...
function evl(t: Term): Sem
  {return evlImpl(t, PMap.nil())}

function evlImpl(t: Term, G: Ctx): Sem {
  switch (t.case) {
    case "uni": return t;
    case "lam": return (s: Sem) => evlImpl(t.bod, PMap.insert(G, t.id, s));
    case "pi": return {
      case: "pi",
      id: t.id,
      dom: evlImpl(t.dom, G),
      bod: (s: Sem) => evlImpl(t.bod, PMap.insert(G, t.id, s))
    }
    case "var": return PMap.lookup(G, t.id);
    case "let": return evlImpl(t.bod, PMap.insert(G, t.id, evlImpl(t.arg, G)));
...
function rfl(T: SemT, t: TermNe, ids: PList.PList<Id>): Sem {
  switch (T.case) {
    case "uni": return t;
    case "pi": return (s: Sem) =>
                rfl(T.bod(s) as SemT, 
                    {case: "app", app: t, arg: rfy(T.dom as SemT, s, ids)},
                    PList.cons(T.id, ids));
    case "app":
    case "var": return t;
  }
...
function rfy(T: SemT, t: Sem, ids: PList.PList<Id>): Term {
  switch (T.case) {
    case "uni": {
      ...
      switch (tT.case) {
        ...
        case "pi": {
          ...
          return {
            case: "pi",
            id: tT.id,
            dom: rfy({case: "uni", lvl: T.lvl}, a, ids),
            bod: rfy({case: "uni", lvl: T.lvl}, b(rfl(a as SemT, {case: "var", id: tT.id}, PList.cons(tT.id, ids))), ids)
          };
        }
        case "app":
        case "var": return tT;
    ...
    case "pi": {
      ...
      let aR: Sem = rfl(a as SemT, {case: "var", id: T.id}, ids);
      return {
        case: "lam",
        id: T.id,
        bod: rfy(b(aR) as SemT, (t as (s: Sem) => Sem)(aR), PList.cons(T.id, ids))
      }
    }
    case "app":
...
export function nrm(T: Term, t: Term): Term
  {return rfy(evl(T) as SemT, evl(t), PList.nil())}
