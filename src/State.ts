...
import { evaluate, evaluateTyp, normalize, normalizeType, reifyTyp } from "./language/Normalization";
    ...
    case "select": {
      // try sig
      {
        ...
        if (hasHoleId(trans.id, buf.t)) {
          let shape = mold(normalizeType(buf.T), normalize(buf.T, buf.t)).get(trans.id) as HoleShape;
          buf.foc = {
            id: trans.id,
            ctx: shape.ctx,
            T: shape.T,
